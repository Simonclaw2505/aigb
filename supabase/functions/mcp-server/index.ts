 ...
 * Authenticates agents via X-API-Key header (SHA-256 hash stored in agent_api_keys)
 */
...
serve(async (req) => {
  ...
  const jsonrpc_error = (id: unknown, code: number, message: string) =>
  ...
  try {
    ...
    if (!rawKey) {
      return json({ jsonrpc: "2.0", id: null, error: { code: -32600, message: "Missing X-API-Key header" } }, 401);
    }
    ...
    const { data: keyRecord, error: keyError } = await supabase
      .from("agent_api_keys")
      .select("id, project_id, organization_id, is_active, expires_at, usage_count, name")
      .eq("key_hash", keyHash)
      .eq("is_active", true)
      .maybeSingle();

    if (keyError || !keyRecord) {
      return json({ jsonrpc: "2.0", id: null, error: { code: -32600, message: "Invalid or inactive API key" } }, 401);
    }
    if (keyRecord.expires_at && new Date(keyRecord.expires_at) < new Date()) {
      return json({ jsonrpc: "2.0", id: null, error: { code: -32600, message: "API key expired" } }, 401);
    }
...
serve(async (req) => {
  ...
  try {
    ...
    if (!rawKey) {
      return json(
        { jsonrpc: "2.0", id: null, error: { code: -32600, message: "Missing X-API-Key header" } },
        401
      );
    }
    ...
    if (keyError || !keyRecord) {
      return json(
        { jsonrpc: "2.0", id: null, error: { code: -32600, message: "Invalid or inactive API key" } },
        401
      );
    }
    ...
    if (keyRecord.expires_at && new Date(keyRecord.expires_at) < new Date()) {
      return json(
        { jsonrpc: "2.0", id: null, error: { code: -32600, message: "API key expired" } },
        401
      );
    }
    ...
    try {
      ...
      return jsonrpc_error(null, -32700, "Parse error: invalid JSON");
    }
    ...
    if (method === "initialize") {
      return json({
        ...
        result: {
          ...
          serverInfo: { name: "AIGB MCP Gateway", version: "1.0.0" },
          capabilities: { tools: {} },
        },
      });
    }
    ...
    if (method === "tools/list") {
      ...
      if (capError) {
        return jsonrpc_error(id, -32603, "Failed to load tools: " + capError.message);
      }

      // deno-lint-ignore no-explicit-any
      const tools = (capabilities ?? []).filter((c: any) => c.action_templates).map((c: any) => {
        const at = c.action_templates;
        const needsApproval = c.agent_policy === "require_confirmation" || c.agent_policy === "require_approval";
        return {
          name: at.id,
          description: (at.description ?? at.name) + (needsApproval ? " [requires human approval]" : ""),
          inputSchema: at.input_schema ?? { type: "object", properties: {} },
        };
      });
    ...
    if (method === "tools/call") {
      ...
      const actionId = p.name;
      const inputs = p.arguments ?? {};

      if (!actionId) return jsonrpc_error(id, -32602, "Missing tool name");
      ...
      if (capErr || !cap) return jsonrpc_error(id, -32602, "Tool not found or not enabled for this project");
      if (cap.agent_policy === "deny") return jsonrpc_error(id, -32603, "Tool execution denied by policy");

      if (cap.agent_policy === "require_approval") {
        const { data: approvalReq } = await supabase
          .from("approval_requests")
          .insert({ project_id: projectId, action_template_id: actionId, input_parameters: inputs, requested_by: "agent", status: "pending" })
          .select("id").single();
        return json({
          jsonrpc: "2.0", id,
          result: { content: [{ type: "text", text: JSON.stringify({ status: "pending_approval", message: "This action requires human approval before execution.", approval_request_id: approvalReq?.id }) }] },
        });

    // 2. Parse JSON-RPC body
    let body: { jsonrpc?: string; method?: string; params?: unknown; id?: unknown };
    try { body = await req.json(); }
    catch { return jsonrpc_error(null, -32700, "Parse error: invalid JSON"); }
    const { method, params, id } = body;
    ...
    if (method === "tools/list") {
      const { data: caps, error: capError } = await supabase
        ...
        .eq("is_active", true)
        .neq("agent_policy", "deny");
      if (capError) return jsonrpc_error(id, -32603, "Failed to load tools: " + capError.message);
      // deno-lint-ignore no-explicit-any
    ...
    if (method === "tools/call") {
      ...
      const { data: cap, error: capErr } = await supabase
        ...
        .eq("is_active", true)
        .maybeSingle();
      if (capErr || !cap) return jsonrpc_error(id, -32602, "Tool not found or not enabled");
      if (cap.agent_policy === "deny") return jsonrpc_error(id, -32603, "Tool execution denied by policy");
